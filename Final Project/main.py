...
========================================
Responsibilities:
  1. Connect to WiFi and advertise mDNS hostname
  2. Run a non-blocking HTTP server on port 80
  3. Route incoming HTTP requests to CarController
  4. Run the main control loop (sensor + motor update)

mDNS
----
After flashing, the car is reachable at:
    http://car4wd.local/

No IP address lookup needed. Change MDNS_HOSTNAME below if running
multiple cars on the same network (e.g. "car4wd-2").

How mDNS works on MicroPython
------------------------------
network.WLAN.config(dhcp_hostname=...) registers the hostname
with the ESP32 WiFi stack during the DHCP exchange. The stack
then answers mDNS (.local) queries on the LAN automatically.
Requires MicroPython ≥ v1.19.

Files to upload to the ESP32 filesystem
-----------------------------------------
    main.py
    car_controller.py
...
All endpoints accept GET requests and return JSON.
  ...
  GET /move?cmd=<forward|backward|left|right|stop>
      Send a discrete movement command.
      Returns 403 if E-STOP lock is active.
      Returns 400 if cmd value is unrecognised.

  GET /stop
      Hard emergency stop — cuts motors AND locks controls.
      Use /resume to unlock.

  GET /resume
      Clear E-STOP lock. Car stays still until next /move.

  GET /light?s=<0|1>
      Neopixel strip off (0) or on (1).

  GET /status
      Full car state as JSON:
        { distance, stopped, locked, light, cmd }

Design notes
------------
  - The HTTP server is non-blocking (server.setblocking(False)).
    conn.settimeout(RECV_TIMEOUT_S) caps each request read at 50ms
    so a slow/dead client cannot freeze the sensor update loop.
  - car.update() is called every loop tick (~15ms effective rate).
    It handles ultrasonic triggering (rate-limited to 80ms) and
    applies the obstacle speed scale to motor PWM.
  - The hardware ECHO IRQ in CarController cuts motor PWM directly
    when distance ≤ 10cm + forward motion, independent of this loop.
"""
