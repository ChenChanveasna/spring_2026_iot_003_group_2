...
temperature = "N/A"
distance = "N/A"
lcd_mode = "idle"   # idle | sensor_temp | sensor_dist | custom
custom_text = ""
scroll_index = 0
last_scroll_time = 0
SCROLL_DELAY = 50   # ms
...
while True:
    ...
    # --- LCD BUTTONS ---
        ...
        lcd_mode = "sensor_temp"
        update_lcd("Temp: {}C".format(temperature))

    elif "/dist" in request:
        lcd_mode = "sensor_dist"
        update_lcd("Dist: {}cm".format(distance))

    # --- CUSTOM LCD TEXT (ROBUST REGEX) ---
    if request.startswith("GET /lcd"):
        # Extract ONLY the request line
        request_line = request.split("\r\n")[0]

        # Regex: capture msg value only
        match = re.search(r"/lcd\?msg=([^ ]+)", request_line)
        if match:
            raw_msg = match.group(1)

            # URL decode (basic, correct for forms)
